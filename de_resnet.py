...
class fuse_a(nn.Module):
    ...
    def __init__(self, in_c):
        ...
        self.conv1x1 = torch.nn.Sequential(
                torch.nn.Conv2d(2 * in_c, 2 * in_c, kernel_size=1, stride=1, padding=0, bias=False),
                torch.nn.BatchNorm2d(2 * in_c),
                torch.nn.ReLU(inplace=True)
            )
    ...
    def forward(self, d, p):
        ...
        con1 = self.conv1x1(p[2])
        fuse_1 = self.conv3x3_simattn(down * sim_a_a +d * (1 - sim_a_a))
        fuse_2 = self.conv3x3_simattn(con1 * sim_a_a +d * (1 - sim_a_a))
        fuse = 0.5 * (fuse_1 + fuse_2)
        fb = torch.cat([d, fuse], dim=1)
        out = self.catconv(fb)
        return out


class fuse_b(nn.Module):
    '''
    inputs: features of encoder block
    outputs: projected features
    '''
    ...
    def forward(self, d, p):
        with torch.no_grad():
            sim_a = torch.unsqueeze(F.cosine_similarity(d.detach(), p[1]), dim=1)
            sim_a_a = sim_a

        down = self.conv1x1_downsample_dim(p[0])
        con1 = self.conv1x1(p[1])
        fuse_1 = self.conv3x3_simattn(down * sim_a_a + d * (1 - sim_a_a))
        fuse_2 = self.conv3x3_simattn(con1 * sim_a_a + d * (1 - sim_a_a))
        fuse = 0.5 * (fuse_1 + fuse_2)
        fb = torch.cat([d, fuse], dim=1)
...
class ResNet_improved(nn.Module):
    ...
    def __init__(
        ...
        super(ResNet_improved, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        self.attention1 = fuse_a(base * 8)
        self.attention2 = fuse_b(base * 4)
    ...
    def _forward_impl(self, x: Tensor, ad, epoch, epoch_therould) -> Tensor:
        # See note [TorchScript super()]
        feature_a = self.layer1(x)
        if epoch < epoch_therould:  #50, 10
            feature_a_ = 0.8 * feature_a + 0.2 * self.attention1(feature_a, ad)
        else:
            feature_a_ = feature_a
        feature_b = self.layer2(feature_a_)
        if epoch < epoch_therould:
            feature_b_ = 0.8 * feature_b + 0.2 * self.attention2(feature_b, ad)  #0.8, 0.2
        else:
            feature_b_ = feature_b
        feature_c = self.layer3(feature_b_) # 128*32*32->64*64*64

        return [feature_c, feature_b, feature_a]

    def forward(self, x: Tensor, ad, epoch, epoch_therould) -> Tensor:
        return self._forward_impl(x, ad, epoch, epoch_therould)
...
def _resnet(
    ...
    **kwargs: Any
    ...
    model = ResNet_improved(block, layers, **kwargs)
    if pretrained:
